...
public class saleProcessFindProduct {
    ...
    public String findProductForm(@PathVariable("cmdId") Long cmdId,Model uiModel,HttpServletRequest httpServletRequest) {
		SaleProcess saleProcess = new SaleProcess(CommandeClient.findCommandeClient(cmdId));
	        uiModel.addAttribute("saleProcess",saleProcess);
	        uiModel.addAttribute("appMessage",httpServletRequest.getAttribute("appMessage"));
    ...
    public String findProductForm(@PathVariable("cmdId") Long cmdId,Model uiModel,HttpServletRequest httpServletRequest) {
        ...
        // List<LigneApprovisionement> search = LigneApprovisionement.fatalSearchAJAX(designation).setMaxResults(100).getResultList();
