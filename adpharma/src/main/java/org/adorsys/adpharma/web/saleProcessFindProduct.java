...
public class saleProcessFindProduct {
	...
	public String findProduct(HttpServletRequest httpServletRequest) {
		...
		if ("on".equals(rp)) {
			search = LigneApprovisionement.findLigneApprovisionnementForSale(designation,null, Etat.CLOS).setMaxResults(200).getResultList();

		}else {
			search = LigneApprovisionement.findLigneApprovisionnementForSale(designation,BigInteger.ONE, Etat.CLOS).setMaxResults(200).getResultList();
